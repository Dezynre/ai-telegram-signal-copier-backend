...
This module sends Telegram-style trading messages to the OpenAI API
and returns extracted trade details as a Python dictionary.
"""
...
# Create the OpenAI client using credentials loaded from config.py.
client = OpenAI(
...
# System instructions used by the model when analyzing trading messages.
# These rules keep the output focused on trade extraction instead of
...
# JSON schema expected from the OpenAI response.
# Keeping this schema strict prevents the parser from returning unexpected
# fields that the backend does not know how to store.
SIGNAL_SCHEMA = {
    "type": "object",
...
def parse_signal_with_openai(raw_text):
    ...
    Args:
        raw_text: Original message text submitted to the backend.

    Returns:
        dict: Parsed trading signal fields returned by the OpenAI API.
    """

    # Fail early when the API key is missing.
    # This gives a clear local error before any API request is attempted.
    ...
    # The model response is JSON text. Convert it into a Python dictionary
    # so the service layer can store individual fields in SQLite.
